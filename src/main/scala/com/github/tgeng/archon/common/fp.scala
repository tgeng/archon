...
object Functor:
  def map[F[_], T, S](f: F[T], g: T => S)(using tc: Functor[F]): F[S] =
    tc.map(f, g)

  inline given derived[F[_]](using m: K1[F]): Functor[F] =
    lazy val functors =
      summonK1AsList[LiftK1[Functor, m.MirroredElemTypes], Functor]
    inline m match
  ...
  given [E]: Functor[Either[E, *]] with
    override def map[A, B](ea: Either[E, A], f: A => B): Either[E, B] =
      ea.map(f)
...
object Applicative:
  ...
  given Applicative[List] with
    ...
    override def starApply[T, S](f: List[T => S], a: List[T]): List[S] =
      f.flatMap(f => a.map(f))

  given Applicative[Option] with
    override def pure[T](t: T): Option[T] = Some(t)

    override def starApply[T, S](f: Option[T => S], a: Option[T]): Option[S] =
      f.flatMap(f => a.map(f))
...
object Monad:
  ...
  given Monad[List] with
    override def flatMap[T, S](a: List[T], g: T => List[S]): List[S] =
      a.flatMap(g)

  given Monad[Option] with
    override def flatMap[T, S](a: Option[T], g: T => Option[S]): Option[S] =
      a.flatMap(g)
...
object Alternative:
  ...
  def or[A[_], T](a: A[T], b: => A[T])(using tc: Alternative[A]): A[T] =
    tc.or(a, b)
