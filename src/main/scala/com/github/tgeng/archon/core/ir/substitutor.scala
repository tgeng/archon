...
/** Local references are represented as DeBruijn indices so `var 0` points to
  * the right most entry in the context. In this setting, a "trivial" mapping
  * should map `var (sourceContextSize - 1)` to the first var in target context
  * (DeBruijn index targetContextSize - 1). [[nonTrivialMapping]] contains the
  * mapping of the last variables in the target context. If target context is
  * longer than source context, then [[nonTrivialMapping]] must account for this
  * gap. In addition, it may go beyond this gap to account for more mapping. To
...
class Substitutor[T: DeBruijn]
  ...
  /** @param boundIndex
    *   up to how many DeBruijn indices in the target context that
    *   materialization should happen. Default value makes materialization
    *   happens fully
    */
