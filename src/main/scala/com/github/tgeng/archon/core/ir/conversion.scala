...
def checkIsConvertible
  ...
  else (left.normalized, right.normalized, ty) match
    ...
    case _ =>
      ctx.withMetaResolved2(left, right):
        ...
        case (CType(upperBound1, eff1), CType(upperBound2, eff2)) =>
            checkIsConvertible(eff1, eff2, Some(EffectsType())) ++ checkIsConvertible(upperBound1, upperBound2, Some(right))
        ...
        case (F(vTy1, eff1, u1), F(vTy2, eff2, u2)) =>
            checkIsConvertible(eff1, eff2, Some(EffectsType())) ++ checkIsConvertible(u1, u2, Some(UsageType())) ++ checkIsConvertible(vTy1, vTy2, None)
