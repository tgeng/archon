  ...
  if left == right then Right(Set.empty)
    ...
    for
      ...
      r <- ty match
        ...
        case _ => ctx.withMetaResolved2(left, right):
          ...
          // convertible terms.
          case ((m1, elims1), (m2, elims2)) if m1 == m2 => Right(Set(Constraint.CConversion(Γ, left, right, ty)))
          ...
          case ((_ : RGuarded, _), _) | (_, (_ : RGuarded, _)) => Right(Set(Constraint.CConversion(Γ, left, right, ty)))
          ...
          case ((u1: RUnsolved, elims1), (u2: RUnsolved, elims2)) =>
            ...
            val (uSmall, elimsSmall, uBig, elimsBig) = if u1.index < u2.index 
