...
enum Pattern(val sourceInfo: SourceInfo) extends SourceInfoOwner[Pattern]:
  ...
  /** Note that matching computation type is prohibited. This should simplify compilation. In
    ...
    *
    * TODO[P2]: type matching as for now is not very useful. Specifically, if the matched type has
    * some upperbound specified, currently elaboration does not honor the constraint. Also, it seems
    * to be useful to allow multiple upperbounds in a `Type` so that a limited form of union type
    * can be supported. (General union type is much more difficult because it would complicates type
    * equality and subtyping). I will keep this as it is for now. But unless we want to expand it
    * with limited union type, we should remove it.
    */
  ...
  // TODO[P2]: add matchers for usage, eq-decidability, and handler type.
