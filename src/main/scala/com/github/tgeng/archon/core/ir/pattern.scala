...
enum Pattern(val sourceInfo: SourceInfo) extends SourceInfoOwner[Pattern]:
  ...
  /** Note that matching computation type is prohibited. This should simplify
    * compilation. In addition, it's unclear how type checking of effects can
  ...
  override def withSourceInfo(sourceInfo: SourceInfo): Pattern =
    ...
    this match
      ...
      case PConstructor(name, args, isZeroUsage) =>
        PConstructor(name, args, isZeroUsage)
      case PForcedConstructor(name, args, isZeroUsage) =>
        PForcedConstructor(name, args, isZeroUsage)
...
enum CoPattern(val sourceInfo: SourceInfo) extends SourceInfoOwner[CoPattern]:
  ...
  case CProjection(name: Name)(using sourceInfo: SourceInfo)
    extends CoPattern(sourceInfo)
...
enum Elimination[T](val sourceInfo: SourceInfo)
  extends SourceInfoOwner[Elimination[T]]:
  case ETerm(v: T)(using sourceInfo: SourceInfo)
    extends Elimination[T](sourceInfo)
  case EProj(n: Name)(using sourceInfo: SourceInfo)
    extends Elimination[T](sourceInfo)
