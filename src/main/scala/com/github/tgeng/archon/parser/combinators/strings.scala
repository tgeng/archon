...
extension[M[+_]]
  ...
  (using Alternative[ParseResult[M, *]])
  (e: P.type)
  def getColumn: ParserT[Char, Int, M] = P.info(
    (input, index) =>
      val lineStart = input.lastIndexOf('\n', index) + 1
        index -lineStart
  )

  def getIndent: ParserT[Char, Int, M] = P.info(
    (input, index) =>
      val lineStart = input.lastIndexOf('\n', index) + 1
  ...
  def quoted(
    ...
    val literal = P.satisfySingle(
      s"<char other than ${
        allEscapeMapping.keys.map(c => s"${escapeSymbol}$c").mkString(
          ""
        )
      }>", !needEscaping(_)
    )
    val special = escapeSymbol >> P.satisfySingle(
      s"<one of ${allEscapeMapping.keySet}>",
      allEscapeMapping.keySet
    ).map(allEscapeMapping)
