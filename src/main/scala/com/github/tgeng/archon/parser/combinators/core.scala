...
object ParseResult:
  def success[M[+_], T](result: M[T], committed: Boolean = false): ParseResult[M, T] =
    ParseResult(result, Seq(), committed)
  def failure[M[+_]: Monad: Alternative, T]
    (errors: Seq[ParseError], committed: Boolean = false)
    : ParseResult[M, Nothing] = ParseResult(Alternative.empty, errors, committed)
  def apply[M[+_], T]
    (result: M[T], errors: Seq[ParseError], committed: Boolean = false)
    : ParseResult[M, T] =
  ...
  private case class WrappedParseResult[M[+_], +T]
    (base: ParseResult[M, T], targetName: Option[String], override val committed: Boolean)
    extends ParseResult[M, T]:
    override def result = base.result
    override def errors =
      base.errors.map(e => ParseError(e.index, e.description, targetName ++: e.targets))
...
extension [I, T, M[+_]](e: P.type)
  inline def apply
    (inline parser: => ParserT[I, T, M], name: String | Null = null, lazily: Boolean = false)
    : ParserT[I, T, M] =
...
extension [I, T, M[+_]]
  (using ap: Applicative[ParserT[I, *, M]])
  (using mm: Monad[M])
  (using am: Applicative[M])
  (using Alternative[M])
  (e: P.type)
  def pure(t: T): ParserT[I, T, M] = ap.pure(t)
  def fail(description: String) = FailureParser(description)

  def satisfy(description: String, action: IndexedSeq[I] => Option[(Int, T)]) =
    new ParserT[I, T, M]:
      override def parseImpl(index: Int)(using input: IndexedSeq[I]): ParseResult[M, (Int, T)] =
...
extension [I, T, M[+_]: Alternative: Monad](p: ParserT[I, T, M])
  infix def asAtom(description: String) = new ParserT[I, T, M]:
    override def parseImpl(index: Int)(using input: IndexedSeq[I]): ParseResult[M, (Int, T)] =
      ...
      if results == Alternative.empty then failure(Seq(ParseError(index, description)), committed)
      else success(results, committed)
  ...
  def unary_! = new ParserT[I, T, M]:
    override def parseImpl(index: Int)(using input: IndexedSeq[I]): ParseResult[M, (Int, T)] =
      ...
      if results == Alternative.empty then parseResult.commit
      else parseResult
  ...
  def ! = new ParserT[I, T, M]:
    override def parseImpl(index: Int)(using input: IndexedSeq[I]): ParseResult[M, (Int, T)] =
      val parseResult = p.parseImpl(index)
      val ParseResult(results, errors, committed) = parseResult: @unchecked
      if results == Alternative.empty then parseResult
      else parseResult.commit
...
private class FailureParser[-I, M[+_]: Monad: Alternative](description: String | Null = null)
  extends ParserT[I, Nothing, M]:
...
given [M[+_]] (using Applicative[ParseResult[M, *]])
  ...
  (using Monad[M])
  (using Alternative[M])
  : Alternative[ParseResult[M, *]] with
...
class ParserCache[I, M[+_]]:
  private val map =
    mutable.WeakHashMap[IndexedSeq[I], mutable.Map[(Any, Int), ParseResult[M, (Int, ?)]]]()
  def load[T]
    (key: Any, index: Int, action: => ParseResult[M, (Int, T)])
    (using input: IndexedSeq[I])
    : ParseResult[M, (Int, T)] =
    map
      .getOrElseUpdate(input, mutable.WeakHashMap())
      .getOrElseUpdate((key, index), action)
      .asInstanceOf[ParseResult[M, (Int, T)]]
