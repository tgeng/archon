...
object Operator:
  def apply(qualifiedName: QualifiedName, fixity: Fixity, nameParts: Seq[Seq[String]]): Operator =
    ...
    if fixity == Closed && nameParts.size == 1 then throw IllegalArgumentException(
...
enum MixfixAst[N, L]:
  ...
  override def toString: String = this match
    case OperatorCall(op, args, nameParts) =>
      ...
      op.fixity match
        case Closed => (interleave(namePartsString, args) :+ namePartsString.last).mkString(
          "[",
          " ",
          "]"
        )
        case Prefix => interleave(namePartsString, args).mkString("[", " ", "]")
        case Postfix => interleave(args, namePartsString).mkString("[", " ", "]")
        case Infix(_) => (interleave(args, namePartsString) :+ args.last).mkString("[", " ", "]")
...
def trimGraph(g: PrecedenceGraph, nameParts: Set[String]): PrecedenceGraph =
  ...
  for oldNode <- g.filter(isNodeRelevant) do
    nodeMap(oldNode) = new PrecedenceNode :
      override val operators: Map[Fixity, Seq[Operator]] = oldNode.operators.map(
        (fixity, operators) => (fixity, operators.filter(
          isOperatorRelevant
        ))
      )
...
(g: PrecedenceGraph, literalParser: ParserT[N, L, M])
  ...
  (using nn: NamePart[N])
  (using cache: ParserCache[N, M]): ParserT[N, MixfixAst[N, L], M] =
  val illegalIdentifierNames = g.flatMap(node => node.operators.values.flatMap(ops => ops.flatMap(op => op.nameParts.flatten))).toSet

  def getNodeTargetName(node: PrecedenceNode) = node.operators.values.flatMap(
  ...
  extension (node: PrecedenceNode)
        ...
        P.cached(
          ...
          for
            ops <- P.lift((node.pUp, node.op(Infix(Associativity.Non)), node.pUp))
              ...
              P.foldRight1[(Operator, List[MixfixAst[N, L]], List[List[N]]), MixfixAst[N, L]](
                pRight,
                P.pure((t, postArg) => OperatorCall(t(0), t(1) :+ postArg, t(2))),
                pUp
              ) |
              P.foldLeft1[MixfixAst[N, L], (Operator, List[MixfixAst[N, L]], List[List[N]])](
                pUp,
    ...
    def pRight: ParserT[N, (Operator, List[MixfixAst[N, L]], List[List[N]]), M] =
      P.cached(
        (node, "right"),
        node.op(Prefix) |
          P.lift((node.pUp, node.op(Infix(Associativity.Right)))).map(
            (preArg, t) => (t(0), preArg +: t(1), t(2))
          )
      )
    ...
    def pLeft: ParserT[N, (Operator, List[MixfixAst[N, L]], List[List[N]]), M] =
      P.cached(
        (node, "left"),
        node.op(Postfix) |
          P.lift((node.op(Infix(Associativity.Left)), node.pUp)).map(
            (t, postArg) => (t(0), t(1) :+ postArg, t(2))
          )
      )
    ...
    def op(fix: Fixity): ParserT[N, (Operator, List[MixfixAst[N, L]], List[List[N]]), M] =
      P.cached(
        (node, fix),
        union(
          node.operators.getOrElse(fix, Seq())
            .map(
              operator => between(expr, operator.nameParts)
        ...
        g.map(
          node =>
            ...
            P.satisfySingle(
              ...
              P(
                node.op(Closed).map((op, args, nameParts) => OperatorCall(op, args, nameParts)),
                getNodeTargetName(node)
              )
        )
      ) ||
